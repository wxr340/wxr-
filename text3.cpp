...
int main(int argc, char *argv[])
{
    ...
    // 初始化视频参数
  cv::VideoCapture cap1("C:\\Users\\27722\\Desktop\\jjjtp\\dog2.mp4");//右图
    cv::VideoCapture cap2("C:\\Users\\27722\\Desktop\\jjjtp\\dog1.mp4");//左图  
while (1)
    {
       
    // 读取第一个视频的第一帧,采集视频帧    image01  视频帧    image1  视频灰度帧;
    cv::Mat image01;Mat image1;
    cap1 >>image01;
    // 读取第二个视频的第一帧，采集视频帧
    cv::Mat image02;Mat image2;
    cap2 >> image02;
  //  if (image01.empty())//采集为空的处理
       //     continue;
   // if (image02.empty())//采集为空的处理
       //     continue;
    ...
    //【9】计算描述符
    featureDetector->compute(image1, keyPoint1, captureDescription1);
    featureDetector->compute(image2, keyPoint2, captureDescription2);
     //【5】基于FLANN的描述符对象匹配
    ...
    Mat first_match;
    drawMatches(image02, keyPoint2, image01, keyPoint1,goodMatches, first_match);
    ...
    for (int i = 0; i<goodMatches.size(); i++)
     {
        ...
        imagePoints1.push_back(keyPoint1[goodMatches[i].trainIdx].pt);
     }
